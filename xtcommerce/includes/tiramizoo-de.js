$(document).ready(function(){
	...
	if (!window.tiramizoo_init) {
		...
		if ($('#tiramizoo-title').length === 1) {
			...
			$('#tiramizoo-title').parent().parent().parent().after('<tr id="tiramizoo-row"><td align="right"><input type="radio" value="on" id="tiramizoo-enabled-on" name="tiramizoo-enabled" /><input type="radio" value="off" id="tiramizoo-enabled-off" name="tiramizoo-enabled" style="display: none;" /></td><td id="tiramizoo-action"><div id="tiramizoo-container"><div id="tiramizoo-action-label"></div> Lieferung am <br /><div id="tiramizoo-action-select-date"></div> <div id="tiramizoo-action-select-time"></div></div></td><td id="tiramizoo-price" style="width:130px"></td><td align="right">&nbsp;</td></tr>');
			...
			$($h).each(function(idx,e){
				...
				$($d[e]).each(function(idx,$t) {
					...
					$('#tiramizoo-select-time-'+($t.datehash)).append('<option value="'+($t.idhash)+'" class="tiramizoo-time tiramizoo-time-'+($t.datehash)+'">'+($t.after)+' - '+($t.before)+' zu '+($t.price)+'</option>');					
			...
			$('#tiramizoo-action-label').html('Schnelle und zuverlässige Lieferung bis zur Haustür durch professionelle Kuriere. Lass es Dir liefern - sofort!');
